...
function funcDrawPolicy() {
  ...
  if (numOfDrawDeck < 3) {
    ...
    for (var _idx9 = 0; _idx9 < policyDeck.length; _idx9++) {
      if (policyDeck[_idx9] != ENACTED) {
        policyDeck[_idx9] = UNUSED;
        numOfDrawDeck++;
      }
    }
  }
  ...
  for (var _idx10 = 0; _idx10 < 3; _idx10++) {
    var btnPolicy = document.getElementById("btnPolicy" + _idx10);
    ...
    curPickedPolicyCards[_idx10] = thisCard;
...
function funcSelectPolicy(idx) {
  ...
  if (event.srcElement.innerHTML == "X") {
    ...
    if (lastCard == true) {
      ...
      for (var _idx11 = 0; _idx11 < 3; _idx11++) {
        if (document.getElementById("btnPolicy" + _idx11).style.display != "none") {
          if (curPickedPolicyCards[_idx11] < 6) {
            numOfEnactedLiberalPolicies++;
          } else {
            numOfEnactedFascistPolicies++;
          }

          policyDeck[curPickedPolicyCards[_idx11]] = ENACTED;
          break;
        }
      }
    ...
    for (var _idx12 = 0; _idx12 < 3; _idx12++) {
      document.getElementById("btnPolicy" + _idx12).innerHTML = "";
    }
